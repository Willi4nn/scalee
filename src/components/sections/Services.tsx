...
export function Services() {
  return (
    ...
    >
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-250 h-75 bg-linear-to-b from-primary/5 to-transparent blur-3xl pointer-events-none" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-5 text-left">
          {SERVICES.map((service, i) => (
            <motion.div
              key={service.num}
              className={cn(
                'relative bg-white border border-border/60 rounded-2xl p-7 sm:p-8 flex flex-col transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.08)] hover:border-border-strong group',
                CARD_SPANS[i]
              )}
              ...
              whileHover={{ y: -6 }}
              transition={{ delay: i * 0.08, duration: 0.1, ease: 'easeOut' }}
            >
              {i === 0 && (
                <span className="absolute top-6 right-6 sm:top-7 sm:right-7 z-20 font-mono text-[0.6rem] font-bold tracking-wide uppercase bg-emerald-50 text-emerald-600 border border-emerald-200/80 px-2.5 py-1 rounded-full">
                  Mais procurado
                </span>
              )}
              ...
              <div className="h-37.5 sm:h-42.5 rounded-2xl mb-7 flex items-center overflow-hidden relative bg-bg-alt/60 border border-border/40 group-hover:bg-bg-alt transition-colors duration-500">
                  ...
                  <div className="relative flex items-center justify-center w-full h-full group-hover:scale-105 transition-transform duration-500 px-6">
                    <div className="w-full h-24 bg-slate-900 rounded-t-xl border-x border-t border-slate-800 shadow-2xl relative overflow-hidden flex flex-col">
                      ...
                      <div className="flex-1 relative">
                        <div className="absolute -top-5 left-1/2 -translate-x-1/2 w-20 h-20 bg-gradient-brand rounded-full blur-[20px] opacity-60 group-hover:opacity-100 transition-opacity" />
                ...
                {i === 2 && (
                  <div className="relative flex items-center justify-center w-full h-full group-hover:scale-105 transition-transform duration-500">
                    <div className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px)] bg-size-[12px_12px]" />
                ...
                {i === 3 && (
                  <div className="relative flex items-end justify-center w-full h-full group-hover:scale-105 transition-transform duration-500 pt-8 px-6">
                    <div className="w-full h-full bg-white rounded-t-xl border-x border-t border-border shadow-[0_-10px_20px_rgba(0,0,0,0.03)] flex overflow-hidden">
                      ...
                      <div className="flex-1 p-3 flex flex-col gap-2">
                        ...
                        <div className="flex gap-2 flex-1">
                          <div className="flex-1 bg-border/30 rounded-lg flex items-end gap-1 p-1.5">
                            ...
                            />
                            <div
                              className="w-1.5 bg-primary/80 rounded-sm"
                              style={{ height: '100%' }}
                            />
                          </div>
              ...
              <a
                href={service.link}
                className="inline-flex items-center gap-2 text-[0.9rem] font-bold text-primary mt-auto relative overflow-hidden w-fit"
              >
